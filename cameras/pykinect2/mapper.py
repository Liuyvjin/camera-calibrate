...
def depth_2_color_space(kinect, depth_space_point, depth_frame_data, show=False, return_aligned_image=False):
    ...
    align_depth_img = np.zeros((1080, 1920, 1), dtype=np.uint16)  # 1C
    align_depth_img[:, :] = depth_img[depthYs, depthXs, :]
    if show:
        cv2.imshow('Aligned Image', cv2.resize(align_depth_img, (int(1920 / 2.0), int(1080 / 2.0))))
        cv2.waitKey(1)
...
if __name__ == '__main__':
    ...
    print(kinect.color_frame_desc.Height, kinect.color_frame_desc.Width)
    print(kinect.depth_frame_desc.Height, kinect.depth_frame_desc.Width)
    print(intrinsics(kinect))
    cnt = 0
    while True:
        if kinect.has_new_depth_frame():
            ...
            depth_img_vis = np.clip(depth_img.astype(np.float32) / 2000, 0, 1) # np.percentile(depth_img.reshape(-1), 90), 0, 1)
            cv2.imshow('Test Depth View', depth_img_vis)
            pts = depth_2_world(kinect, kinect._depth_frame_data, _CameraSpacePoint, True)
            pts = pts.reshape(-1, 3)
            ...
            colorImage = color_2_depth_space(kinect, _ColorSpacePoint, kinect._depth_frame_data, show=True, return_aligned_image=True)
        ...
        if k & 0xff == ord('q'):
            break
        if k & 0xff == ord('s'):
            cv2.imwrite('data/color{:04d}.png'.format(cnt), colorImage)
            cv2.imwrite('data/depth{:04d}.png'.format(cnt), depth_img)
            np.save('data/pc{:04d}.npy'.format(cnt), pts)
            cnt += 1
