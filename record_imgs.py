...
def increment_path(path, name='color_', suffix='png'):
    ...
    for n in range(0, 9999):
        if not os.path.exists(f'{path}\{name}{n}.{suffix}'):  #
            break
    return n
IMG_ENV = ''
IMG_ID = increment_path(IMG_PATH, name = f'color{IMG_ENV}_')

#  click mouse to save img
def onMouse(event, x, y, flags, param):
    """ 鼠标点击存图 """
    global IMG_ID
    if event == cv2.EVENT_MBUTTONDOWN:
        cv2.imwrite(f'{IMG_PATH}\color{IMG_ENV}_{IMG_ID}.png', color_img)
        cv2.imwrite(f'{IMG_PATH}\depth{IMG_ENV}_{IMG_ID}.png', depth_img)
        np.save(f'{IMG_PATH}\pc{IMG_ENV}_{IMG_ID}.npy', pts)
        print(f'Saved at {IMG_PATH}\color{IMG_ENV}_{IMG_ID}')
        IMG_ID += 1
